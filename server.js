...
// ============ Mémoire persistante d'Ary (fichier JSON) ============
// Note : sur Render (plan gratuit), ce fichier survit aux redémarrages mais est
// remis à zéro à chaque nouveau déploiement (disque non persistant entre builds).
// Pour une persistance garantie même après un redéploiement, il faudra migrer
// vers une vraie base de données plus tard (ex : SQLite sur disque payant, ou
// un service comme Turso/Supabase en gratuit).
...
function logEvent(type, detail) {
  ...
  data.events.push({ type, detail, at: new Date().toISOString() });
  if (data.events.length > 200) data.events = data.events.slice(-200); // garde les 200 derniers
  saveData(data);
}

// Personnalité d'Ary : chaleureux, motivant, tutoiement — appliquée aux messages
// qu'Ary adresse directement à Hardy (pas au contenu des documents générés,
// qui doit rester dans le registre demandé : juridique, professionnel, etc.)
const ARY_PERSONALITY = "Tu es Ary, l'assistant personnel de Hardy dans Bridge AI. Tu le tutoies toujours, ton ton est chaleureux et motivant comme un pote qui pousse à avancer, jamais moralisateur ni ronflant. Tu restes toujours rigoureux sur les faits : tu n'inventes jamais une information.";

app.get('/api/status', (req, res) => {
  res.send('ARI backend en ligne.');
});

// ---- Mémoire : résumé du contexte + petit mot d'Ary ----
app.get('/api/ary/memory', (req, res) => {
  const data = loadData();
  const events = data.events || [];
  const missions = data.missions || [];

  const findCount = events.filter(e => e.type === 'find').length;
  const createCount = events.filter(e => e.type === 'create').length;
  const paidMissions = missions.filter(m => m.status === 'recu');
  const totalNet = paidMissions.reduce((sum, m) => {
    const fee = { Payoneer: 0.02, Wise: 0.01, PayPal: 0.025 }[m.channel] ?? 0.02;
    return sum + m.amount * (1 - fee);
  }, 0);

  res.json({
    summary: {
      recherches_lancees: findCount,
      documents_generes: createCount,
      missions_reglees: paidMissions.length,
      total_net_gagne_eur: Math.round(totalNet * 100) / 100
    },
    recent_events: events.slice(-10)
  });
});

// Petit mot motivant d'Ary, calculé localement (pas d'appel IA, instantané et gratuit)
app.get('/api/ary/greeting', (req, res) => {
  const data = loadData();
  const missions = data.missions || [];
  const now = new Date();
  ...
  const weekNet = missions
    .filter(m => m.status === 'recu' && new Date(m.date) >= weekStart)
    .reduce((sum, m) => {
      const fee = { Payoneer: 0.02, Wise: 0.01, PayPal: 0.025 }[m.channel] ?? 0.02;
      return sum + m.amount * (1 - fee);
    }, 0);
  ...
  const target = 75;
  const pct = Math.min(100, Math.round((weekNet / target) * 100));

  let message;
  if (missions.length === 0) {
    message = "Salut Hardy 👋 Prêt à lancer ta première recherche avec ARI FIND ? Chaque mission commence par une opportunité trouvée.";
  } else if (pct >= 100) {
    message = `Objectif de la semaine atteint (${weekNet.toFixed(0)}€) 🎉 Tu peux viser encore plus haut ou souffler un peu, tu l'as mérité.`;
  } else if (pct >= 50) {
    message = `Tu es à ${pct}% de ton objectif hebdo (${weekNet.toFixed(0)}€ / ${target}€). Encore un petit effort, tu y es presque.`;
  } else {
    message = `${weekNet.toFixed(0)}€ cette semaine sur ${target}€ visés. On continue, une mission à la fois.`;
  }

  res.json({ message, pct, weekNet: Math.round(weekNet * 100) / 100, target });
});

// ---- ARI PAY : missions persistées côté serveur ----
app.get('/api/pay/missions', (req, res) => {
  const data = loadData();
  res.json({ missions: data.missions || [] });
});

app.post('/api/pay/missions', (req, res) => {
  const { client, amount, currency, channel, status, date } = req.body || {};
  if (!client || !amount || amount <= 0) {
    return res.status(400).json({ error: "Client et montant valides requis." });
  }
  const data = loadData();
  ...
  const mission = {
    ...
    channel: channel || 'Payoneer',
    status: status || 'recu',
    date: date || new Date().toISOString().slice(0, 10)
  };
  data.missions.push(mission);
  saveData(data);
  logEvent('payment', { client: mission.client, amount: mission.amount, status: mission.status });
  res.json({ missions: data.missions });
});

app.delete('/api/pay/missions/:index', (req, res) => {
  const idx = parseInt(req.params.index, 10);
  const data = loadData();
  data.missions = data.missions || [];
  if (isNaN(idx) || idx < 0 || idx >= data.missions.length) {
...
Donne entre 3 et 8 opportunités dans "opportunities", classées par match_score décroissant. Si un ciblage a été demandé, donne entre 2 et 5 opportunités dans "targeted_opportunities". Sinon renvoie "targeted_opportunities": [].`;
  ...
  try {
    ...
    logEvent('find', {
      competences: (competences || '').slice(0, 120),
      resultats: parsed.opportunities.length,
      cible: parsed.targeted_opportunities.length
    });
...
app.post('/api/create', async (req, res) => {
  ...
  try {
    ...
    logEvent('create', {
      ...
      titre: (effectiveFields.title || effectiveFields.platform || '').slice(0, 120)
    });
